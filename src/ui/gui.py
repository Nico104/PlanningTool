...
# from PySide6.QtWidgets import QApplication, QMainWindow, QMessageBox, QDialog, QWidget, QInputDialog
# from PySide6.QtGui import QAction, QPalette, QActionGroup
...
#         # Optional: Layouts verwalten
#         self.act_save_layout = QAction("Aktuelles Layout speichern…", self)
#         self.act_save_layout.triggered.connect(self._save_layout_dialog)
#         self.layout_menu.addAction(self.act_save_layout)

#         self.act_reset_layouts = QAction("Layouts zurücksetzen", self)
#         self.act_reset_layouts.triggered.connect(self._reset_default_layouts)
#         self.layout_menu.addAction(self.act_reset_layouts)

#         self.layout_menu.addSeparator()
#         # Hier kommen die Layout-Items rein (dynamisch)
...
#     #layout
#     def _init_layout_system(self):
#         # 1) Default Layout nach dem Dock-Setup speichern
#         self._layouts["Standard"] = self.saveState()
#         self._current_layout_name = "Standard"

#         # 2) Menüeinträge aufbauen
#         self._rebuild_layout_menu_items()

#     def _rebuild_layout_menu_items(self):
#         # Alles nach dem Separator löschen (wir haben oben Actions + Separator)
#         # -> easiest: Menü leeren und neu aufbauen
...
#         # ActionGroup neu (damit checked sauber ist)
#         self.layout_group = QActionGroup(self)
#         self.layout_group.setExclusive(True)
...
#         if not state:
#             return
#         ok = self.restoreState(state)
#         if ok:
#             self._current_layout_name = name
#             # optional: sicherstellen, dass checks passen
#             self._rebuild_layout_menu_items()

#     def _save_layout_dialog(self):
#         name, ok = QInputDialog.getText(self, "Layout speichern", "Name für Layout:")
#         if not ok or not name.strip():
#             return
#         name = name.strip()

#         self._layouts[name] = self.saveState()
#         self._current_layout_name = name
#         self._rebuild_layout_menu_items()

#     def _reset_default_layouts(self):
#         # Behält nur Standard
...
#     qss_path = Path(__file__).parent / "styles" / "light.qss"
