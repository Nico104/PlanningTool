...
from ...models.models import Raum, Termin
...
class PlannerDayView:
    ...
    def _build_day_grid(self, rooms: List[Raum], terms: List[Termin], d: date, sem: Optional[str]) -> None:
        ...
        for t in terms:
            ...
            r0 = max(0, min(row_for(t.zeit.von), len(times) - 1))
            r1 = max(r0 + 1, min(row_for(t.zeit.bis), len(times)))
            span = max(1, r1 - r0)
            ...
            if any(occupied[rr][c] for rr in range(r0, min(len(times), r0 + span))):
                ...
                conflict_line = f"⚠ KONFLIKT: {fmt_time(t.zeit.von)}–{fmt_time(t.zeit.bis)} {t.typ} {t.lva_id} {lva_name}"
